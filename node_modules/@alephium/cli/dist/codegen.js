...
exports.codegen = exports.sortByName = exports.genLoadDeployments = void 0;
...
function formatParameters(fieldsSig) {
    return fieldsSig.names
        .map((name, idx) => (name === web3_1.StdIdFieldName ? '' : `${name}: ${toTsType(fieldsSig.types[`${idx}`])}`))
        .filter((str) => str !== '')
        .join(', ');
}
...
function genCallMethods(contract) {
...
function genTxCallMethods(contract) {
    const functions = contract.functions;
    if (functions.length === 0) {
        return '';
    }
...
function genTxCallMethod(contractName, functionSig) {
...
function contractFieldType(contractName, fieldsSig) {
    const hasFields = fieldsSig.names.length > 0;
    return hasFields ? `${contractTypes(contractName)}.Fields` : '{}';
}
function importRalphMap(contract) {
    if (contract.mapsSig === undefined)
        return '';
    return `import { RalphMap } from '@alephium/web3'`;
}
function genMaps(contract) {
    ...
    const maps = mapsSig.names.map((name, index) => {
        ...
        const [key, value] = (0, web3_1.parseMapType)(mapType);
        return `${name}: new RalphMap<${toTsType(key)}, ${toTsType(value)}>(${contract.name}.contract, this.contractId, '${name}')`;
    });
...
function nodeValToString(value) {
    return valToString((0, web3_1.fromApiPrimitiveVal)(value, value.type));
}
...
function genEventIndex(contract) {
...
function genLocalConsts(contract) {
...
function genConstants(constants, isLocal) {
...
function genEnum(enumDef, isLocal) {
    const prefix = isLocal ? '' : 'export const ';
    const separator = isLocal ? ': ' : ' = ';
...
function genEnums(enums, isLocal) {
    return enums.map((enumDef) => genEnum(enumDef, isLocal));
}
function genGetContractEventsCurrentCount(contract) {
    if (contract.eventsSig.length === 0) {
        return '';
    }
    return `
...
function genSubscribeAllEvents(contract) {
...
function genEncodeFieldsFunc(contract) {
...
function genTestMethod(contract, functionSig) {
    ...
    const hasMapVars = contract.mapsSig !== undefined;
    ...
    const baseRetType = tsReturnTypes.length === 0
        ? 'null'
...
function genTestMethods(contract) {
    return `
    tests = {
...
function genStateForTest(contract, fieldType) {
...
function genCallMethodTypes(contract) {
...
function genSignExecuteMethodTypes(contract) {
    const entities = contract.functions.map((functionSig) => {
        const funcHasArgs = functionSig.paramNames.length > 0;
        const params = funcHasArgs
...
function genMulticall(contract) {
...
function getContractFields(contract) {
    const stdIdFieldIndex = contract.fieldsSig.names.findIndex((name) => name === web3_1.StdIdFieldName);
...
function genContract(contract, artifactRelativePath) {
...
function genScripts(outDir, artifactDir, exports) {
...
function genContractByCodeHash(outDir) {
    const source = `
    ${header}
...
function genContracts(outDir, artifactDir, exports) {
...
async function getAllDeployments(config) {
    const filePaths = web3_1.networkIds
        .map((n) => (0, utils_1.getDeploymentFilePath)(config, n))
        .filter((filePath) => fs_1.default.existsSync(filePath));
    const allDeployments = [];
    for (const filePath of filePaths) {
        const deployments = await deployment_1.Deployments.from(filePath);
        allDeployments.push(...deployments.deployments);
    }
    return allDeployments;
}
function genToDeployments(contracts, optionalContracts, allScripts) {
    const contractInstances = contracts.map((taskId) => {
        const typeName = getTypeFromTaskId(taskId);
        return `
    ${(0, utils_1.taskIdToVariable)(taskId)}: {
    ...
    const optionalContractInstances = optionalContracts.map((taskId) => {
        const typeName = getTypeFromTaskId(taskId);
        return `
    ${(0, utils_1.taskIdToVariable)(taskId)}: json.contracts['${taskId}'] === undefined ? undefined : {
      ...json.contracts['${taskId}'],
      contractInstance: ${typeName}.at(json.contracts['${taskId}'].contractInstance.address)
    }
    `;
    });
    const scripts = allScripts.map((taskId) => `${(0, utils_1.taskIdToVariable)(taskId)}: json.scripts['${taskId}']`).join(',');
...
function genContractField(taskId, optional) {
    const typeName = getTypeFromTaskId(taskId);
    const varName = (0, utils_1.taskIdToVariable)(taskId);
    return `${varName}${optional ? '?' : ''}: DeployContractExecutionResult<${typeName}Instance>`;
}
function genScriptField(taskId, optional) {
    const varName = (0, utils_1.taskIdToVariable)(taskId);
    return `${varName}${optional ? '?' : ''}: RunScriptResult`;
}
function genDeploymentsType(allDeployments) {
...
function getRelativePath(config, networkId, fromPath) {
    const deploymentFilePath = (0, utils_1.getDeploymentFilePath)(config, networkId);
    if (!fs_1.default.existsSync(path_1.default.resolve(deploymentFilePath))) {
        return undefined;
    }
    return path_1.default.relative(fromPath, deploymentFilePath);
}
function getTypeFromTaskId(taskId) {
    return taskId.split(':')[0];
}
async function genLoadDeployments(config) {
    const tsPath = path_1.default.join(config.artifactDir ?? types_1.DEFAULT_CONFIGURATION_VALUES.artifactDir, 'ts');
    if (!fs_1.default.existsSync(tsPath)) {
        fs_1.default.mkdirSync(tsPath, { recursive: true });
    }
    const allDeployments = await getAllDeployments(config);
    const contractInstanceTypes = dedup(allDeployments.flatMap((d) => Array.from(d.contracts.keys()).map((taskId) => getTypeFromTaskId(taskId))))
        .map((contractName) => `${contractName}, ${contractName}Instance`)
        .join(',');
    const deploymentsPath = web3_1.networkIds.map((n) => [n, getRelativePath(config, n, tsPath)]);
  ...
  `;
    const deploymentsFilePath = path_1.default.join(tsPath, 'deployments.ts');
    formatAndSaveToFile(deploymentsFilePath, source);
}
exports.genLoadDeployments = genLoadDeployments;
function formatAndSaveToFile(filepath, code) {
    const source = prettier.format(code, { parser: 'typescript' });
    fs_1.default.writeFileSync(path_1.default.resolve(filepath), source, 'utf8');
}
function sortByName(artifacts) {
    return artifacts.sort((a, b) => (a.artifact.name > b.artifact.name ? 1 : -1));
}
exports.sortByName = sortByName;
function cleanCode(outDir) {
...
function genStructTypes(outDir) {
    const structs = currentProject.structs;
    if (structs.length === 0)
        return;
    ...
    ${interfaces.join('\n')}
  `;
    const sourcePath = path_1.default.join(outDir, 'types.ts');
    formatAndSaveToFile(sourcePath, sourceCode);
}
function genGlobalConstants(project, outDir) {
    const constants = project.constants;
    const enums = project.enums;
    if (constants.length === 0 && enums.length === 0)
        return;
    const sourceCode = `
    ${header}
  ...
  `;
    const sourcePath = path_1.default.join(outDir, 'constants.ts');
    formatAndSaveToFile(sourcePath, sourceCode);
}
function codegen(project) {
    currentProject = project;
    const artifactDir = project.artifactsRootDir;
    const outDirTemp = path_1.default.join(artifactDir, 'ts');
